...
function main() {
    ...
    scene.background = new THREE.Color("purple"); // Mengatur background menjadi warna ungu
    const fog = new THREE.Fog("lightblue", 1, 100); // Mengatur kabut
    ...
    // Buat tabung
    const cylinderRadiusTop = 2; // Jari-jari atas tabung
    const cylinderRadiusBottom = 2; // Jari-jari bawah tabung
    const cylinderHeight = 6; // Tinggi tabung
    const radialSegments = 32; // Jumlah segmen radial
    ...
    const cubeTextureMap = textureLoader.load('image/kotak_texture.jpg'); // Ganti dengan jalur ke tekstur kubus
    const cubeMaterial = new THREE.MeshStandardMaterial({
        map: cubeTextureMap // Menambahkan tekstur ke kubus
    });

    // Material bola dengan normal map
    const sphereNormalMap = textureLoader.load('image/texture_kertas.jpeg'); // Memuat normal map
    ...
    const sphereMaterial = new THREE.MeshStandardMaterial({
        color: 'coral', // Mengubah warna bola
        normalMap: sphereNormalMap // Menambahkan normal map
    });

    // Material bidang dengan tekstur dan normal map
    const planeTextureMap = textureLoader.load('image/pebble.jpeg'); // Memuat tekstur bidang
    ...
    // Material untuk tabung (warna pink)
    const cylinderMaterial = new THREE.MeshStandardMaterial({
        color: 'pink' // Warna tabung menjadi pink
    });

    // MESHES
    const cube = new THREE.Mesh(cubeGeometry, cubeMaterial); // Menggabungkan geometris dan material
    cube.position.set(2, 10, 0); // Posisi kubus
    ...
    sphere.position.set(-sphereRadius - 2, 10, 0); // Posisi bola
    ...
    const cylinder = new THREE.Mesh(cylinderGeometry, cylinderMaterial); // Menggabungkan geometri dan material untuk tabung
    cylinder.position.set(8, 10, 0); // Posisi tabung di sebelah kanan kubus
    scene.add(cylinder); // Menambahkan tabung ke scene
    ...
    // LIGHTS
    const color = 0xffffff; // Warna cahaya
    ...
    const ambientColor = 0xffffff; // Warna cahaya ambient
    ...
    // DRAW
    function draw(time) {
        time *= 0.015; // Mengubah waktu menjadi detik
        ...
        // Rotasi menyilang tabung
        cylinder.rotation.x += 0.03; // Rotasi pada sumbu x (horizontal)
        cylinder.rotation.y += 0.03; // Rotasi pada sumbu y (vertikal)
